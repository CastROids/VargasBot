...
public class Vendedor extends Perfil {
    ...
    public void realizarVenta(Inventario inventario) throws InputMismatchException {
        int idVenta = listaVentas.size();
        int idProducto = -1, indice;
        String respuesta = "1";
        ...
        do {
            ...
            Producto producto = new Producto(inventario.getListaProductos().get(indice).getIdProducto(),
                    ...
                    inventario.getListaProductos().get(indice).getDescripcion(),
                    inventario.getListaProductos().get(indice).getPrecio(),
                    inventario.getListaProductos().get(indice).getCategoria(),
                    inventario.getListaProductos().get(indice).getStock());
        ...
        for (int i = 0; i < nuevaVenta.getListaProductos().size(); i++) {
            for (int j = 0; j < inventario.getListaProductos().size(); j++) {
                if (nuevaVenta.getListaProductos().get(i).getIdProducto() == inventario.getListaProductos().get(j)
                        .getIdProducto()) {
                    int nuevoStock = inventario.getListaProductos().get(j).getStock()
                            - nuevaVenta.getListaCantidades().get(i);
        ...
        System.out.println("Total: " + nuevaVenta.calcularPrecioTotal());
    }
    ...
    public void consultarHistorialVentas() {
        ...
        for (int i = 0; i < listaVentas.size(); i++) {
            ...
            for (int j = 0; j < listaVentas.get(i).getListaProductos().size(); j++) {
                System.out.println("Producto: " + listaVentas.get(i).getListaProductos().get(j).getNombre() + "\tx"
                        + listaVentas.get(i).getListaCantidades().get(j) + "\n");
            }
